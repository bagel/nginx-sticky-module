...
static char *ngx_http_sticky_set(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	...
	for (i = 1; i < cf->args->nelts; i++) {
		...
		if ((u_char *)ngx_strstr(value[i].data, "text=") == value[i].data) {
			...
			/* is name=ipaddr */
			if (ngx_strncmp(tmp.data, "ipaddr", sizeof("ipaddr") - 1) == 0 ) {
				text = ngx_http_sticky_misc_text_ipaddr;
				continue;
			}
			...
			ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "wrong value for \"text=\": raw, ipaddr, md5 or sha1");
			return NGX_CONF_ERROR;
		}
